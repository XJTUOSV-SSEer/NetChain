...
int experiment::test_VO_size(Response& response){
    ...
    std::map<int, SkipListProof>& VO2 = response.VO2;
    ...
    for(std::pair<int, MHTProof> pf: VO){
        ...
        // 若是叶结点且match，包含 value, acc, --->  bytes
    ...
    // 计算VO2 size
    // 每个SkipListProof中，默认为有1个填充的是acc，其余填充的是digest
    for(std::pair<int, SkipListProof> pf: VO2){
        total_size = total_size + 32 + 4*32 + pf.second.proof.a.length() + pf.second.proof.d.length();
    }
...
int experiment::test_ADS_size(Block blk){
    ...
    SkipList& skiplist = blk.skiplist;
    std::vector<SkipListNode>& vec = skiplist.vec;
    int total_size = 0;
